...
export default function QnaDetailPage() {
  ...
  const { data: question, isLoading, error } = useGetQuestionById(id);
  const { mutate: deleteQuestion, isPending: isDeleting } = useDeleteQuestion();

  const handleEdit = () => {
    router.push(`/Customer/qna/write?edit=${id}`);
  };
      ...
      <div className="w-full max-w-4xl mx-auto px-[20px] mt-[40px]">
        ...
        <div className="w-full min-h-[570px] border border-[#e3e3e3] shadow-lg flex flex-col items-center justify-start px-[20px] py-[20px]">
          ...
          <div className="w-full min-h-[48px] flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-[#e3e3e3] pb-4 sm:pb-0">
            ...
            <div className="flex items-center gap-2">
              <p className="text-gray-600 text-sm whitespace-nowrap">
                <UserNameDisplay userId={question.author_id} />
              </p>
              <span className="text-gray-400">|</span>
              <p className="text-gray-600 text-sm whitespace-nowrap">
                {formatToKoreanDate(question.created_at)}
              </p>
            </div>
          </div>
          ...
          <div className="w-full mt-6">
            ...
            {question.img_url &&
              Array.isArray(question.img_url) &&
              question.img_url.length > 0 && (
                <div className="mb-6 space-y-4">
                  {question.img_url.map((imageUrl, index) => {
                    if (
                      !imageUrl ||
